...
class WimSaleSubscriptionMapper(Component):
    ...
    def map_valid_from(self, record):
        return {"validFrom": record.start_date.strftime(WIM_DATE_FORMAT)}

    def map_valid_until(self, record):
        return {"validUntil": record.end_date.strftime(WIM_DATE_FORMAT)}

    def map_active(self, record):
        return {"active": record.subscription_state in SUBSCRIPTION_PROGRESS_STATE}

    def map_start_of_renewal_period(self, record):
        date = record.next_online_renewal_date
        res = {"startOfRenewalPeriod": date.strftime(WIM_DATE_FORMAT) if date else ""}
        return res

    # TODO : If needs to be updated, update trigger_fields() as well
    @changed_by(
        "start_date", "end_date", "subscription_state", "next_online_renewal_date"
    )
    @mapping
    def map_customer_number(self, record):
        ...
        if not customer_number:
            raise UserError(
                "Customer number doesn't exist: %s" % record.partner_id.name
            )
