...
def _move_specifc_addons_to_local_src(ctx, specific_addon_paths_to_keep):
    ...
    ask_or_abort(
        "These modules will be moved into odoo/local-src. Are you sure it's correct?"
    )

    for path, module in modules_to_move:
        shutil.move(os.path.join(path, module), os.path.join("odoo/local-src/", module))
...
def _prepare_files_and_dirs(ctx, paths_to_keep):
    ...
    files_directories_to_keep = ["./.git"] + GIT_IGNORES + git_ignores_global(ctx)
    if paths_to_keep:
    ...
    ask_or_abort("These directories will be deleted. Are you sure it's correct?")
...
def _prepare_files_to_sync_from_odoo_template(
    ...
    with cd(template):
        ...
        ask_or_abort("These files will be synchronized. Are you sure it's correct?")
    ...
    ask_or_abort(
        "These modules will be entirely overridden. Are you sure it's correct?"
    )
...
def _do_convert_project(
    ...
    ctx.run("git add .")
    msg = "Convert project to new version: {}".format(new_cc_context["odoo_version"])
    ctx.run('git commit -m "{}" -e -vv'.format(msg), pty=True)
...
def _make_old_modules_uninstallables(ctx, old_modules):
    ...
    for old_module in old_modules:
        if os.path.isfile("%s/__manifest__.py" % old_module):
            ...
            file = " {old_module}/__manifest__.py".format(old_module=old_module)
...
def convert_project(
    ...
    (
        ...
        files_to_delete,
        directories_to_delete,
    ) = _prepare_files_and_dirs(ctx, paths_to_keep)
    ...
    with tempdir() as tmp:
        (
            ...
            template,
            new_modules,
        ) = _prepare_files_to_sync_from_odoo_template(
...
def reduce_db_size(ctx, database_name, date):
    ...
    tables_file = open("tasks/migrate_reduce_db_size_tables.txt")
    ...
    for table_to_clean in tables_to_clean:
        ...
        print(
            "==> Clean table {}: {} deleted".format(table_to_clean, record_count[0][0])
        )
