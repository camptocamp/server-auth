...
def demo_to_sample(ctx):
    ...
    print("- odoo/songs/install/data_demo.py (path 'data/demo' to 'data/sample')")
    print("- test.yml")

    print()
    print("The following files were moved:")
    print("- odoo/data/demo to odoo/data/sample")
    print("- odoo/songs/install/data_demo.py to odoo/songs/sample/data_sample.py")
    print()
...
def split_pending_merges(ctx):
    ...
    if not os.path.exists(build_path(old_path)):
        exit_msg(
            "No file found at {}.\nNothing to do here!".format(build_path(old_path))
        )
    ...
    with open(build_path(old_path)) as pending_merges_file:
        data = yaml_load(
            "\n".join(
                line for line in pending_merges_file.read().splitlines() if line.strip()
            )
        )
        for submodule_relpath in data:
            submodule_merges_path = Repo.build_submodule_merges_path(submodule_relpath)
