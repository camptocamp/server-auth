...
def _add_comment_py(path, comment):
    ...
    insert_at = 0
    if content:
        if "coding:" in content[0]:
            insert_at = 1
        elif content[0].startswith("#!"):
            insert_at = 1
            if len(content) > 1 and "coding:" in content[1]:
                insert_at = 2
...
def sync(ctx, commit=True, version=None, fork=None, fork_url=None):
    ...
    if os.getenv("PROJECT_SYNC_FORCE"):
        ...
        ask_or_abort("Selected version: {}. Are you sure it's correct?".format(version))
...
def _do_sync(ctx, cc_context, version, git_repo, commit=True):
    ...
    with tempdir() as tmp:
        ...
        with cd(template):
            ...
            with open(os.path.join(template, ".sync.yml")) as syncfile:
